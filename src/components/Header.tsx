...
import logosblack from '@/assets/logos/logosblack.svg'
...
function Header() {
  ...
  return (
    <header className={`${pathname === '/' && 'bg-[#F9F9F9]'} pt-4 ${sticky && "sticky"}`}
    >
      <div
        className={'flex items-center w-[95vw] justify-between rounded-[1.6rem] py-[1.6rem] lg:px-[3rem] px-[2rem] mx-auto z-20'}>
        <Link href={'/'}>
          <Image src={logosblack} alt={"Logo style black"}/>
        </Link>
        <nav>
          <ul
            className={`fixed z-20 top-0 left-0 w-full h-full flex flex-col gap-[4rem] justify-center text-[2rem] ${isMenuOpen ? '  translate-x-0' : 'translate-x-full'} transition-all lg:bg-none lg:translate-x-0 lg:relative lg:flex items-center lg:flex-row lg:gap-[2.4rem] font-medium lg:text-[1.5rem] text-dark`}>
            <Image src={logosblack} alt={"Logo style black"} className={"lg:hidden absolute top-12 left-10"}/>
